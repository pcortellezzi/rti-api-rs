...
mod compliance {
    ...
    fn extract_text_via_poppler(pdf_path: &Path) -> Option<String> {
        ...
        error!(
            "Could not find or execute pdftotext.exe. Please install poppler or update the path."
        );
        None
    }
    ...
    fn extract_specs_from_pdf() -> Vec<ApiSpec> {
        ...
        if !path.exists() {
            warn!(
                "Skipping compliance test: Reference_Guide.pdf not found at {:?}",
                path
            );
            return vec![];
        }
                    ...
                    if let Some(caps_dir) = re_dir_only.captures(next_line) {
                        if let Ok(id) = caps_id[1].parse::<i32>() {
                            if id >= 10 {
                                ...
                                let direction = format!("From {}", direction_raw);
                                let final_name =
                                    current_name_fragments.join(" ").trim().to_string();
    ...
    fn get_implemented_requests() -> HashMap<i32, String> {
        let content =
            fs::read_to_string("src/api/sender_api.rs").expect("Failed to read sender_api.rs");
        let re = Regex::new(r"let\s+(?:mut\s+)?req\s*=\s*(\w+)\s*\{[\s\S]*?template_id:\s*(\d+)")
            .unwrap();
    ...
    fn get_implemented_responses() -> HashMap<i32, String> {
        let content =
            fs::read_to_string("src/api/receiver_api.rs").expect("Failed to read receiver_api.rs");
    ...
    fn verify_api_coverage() {
        ...
        println!(
            "{:^5} | {:<45} | {:<10} | {:<35} | {:^10}",
            "ID", "TEMPLATE NAME (PDF)", "DIR", "PROTO MESSAGE", "STATUS"
        );
        println!(
            "{:-<5} | {:-<45} | {:-<10} | {:-<35} | {:-<10}",
            "", "", "", "", ""
        ...
        for spec in &specs {
            ...
            let proto_match = if let Some(impl_name) = impl_entry {
                ...
                proto_defs
                    .iter()
                    .find(|p| normalize_proto_name(p) == expected_name)
                    .cloned()
            };
            ...
            let (status_icon, proto_display) = match impl_entry {
                Some(impl_name) => {
                    ...
                    if exact_match {
                        ...
                        if let Some(p_found) = proto_defs
                            .iter()
                            .find(|p| normalize_proto_name(p) == n_impl)
                        {
                ...
                None => {
                    ...
                    let wrong_map_entry = if is_client {
                        implemented_resps.get(&spec.id)
                    } else {
                        implemented_reqs.get(&spec.id)
                    };
        ...
        let mut extra_ids: Vec<i32> = implemented_all
            .keys()
            .cloned()
        ...
        for id in extra_ids {
            ...
            additional_defs_for_print.push((
                format!("{}", id),
                name.clone(),
                "Implemented (Extra)",
            ));
        ...
        let mut undocumented_protos: Vec<String> = proto_defs
            .iter()
        ...
        if failure_detected {
            panic!(
                "Compliance check failed: mismatched names, missing definitions, or extra implementations detected."
            );
        }
    }

...
use rti_api_rs::{
    BracketOrderParams, BracketType, MarketDataField, OrderDuration, OrderParams, PriceType,
    RithmicClient, RithmicMessage, TransactionType, connection_info::RithmicCredentials,
};
use std::env;
...
async fn main() -> Result<(), Report> {
    ...
    let credentials = RithmicCredentials::new(username, password, system, gateway);
    ...
    info!(
        "Connecting to {} as {}",
        credentials.system_name, credentials.user
    );
        ...
        while let Some(response) = event_rx.recv().await {
            match response.message {
                ...
                RithmicMessage::RithmicOrderNotification(notify) => {
                    info!(
                        "Order Update: {:?} Status: {:?}",
                        notify.basket_id, notify.status
                    );
                }
                _ => {
